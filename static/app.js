document.addEventListener('DOMContentLoaded', () => {
    ...
    runBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Render Opinions
            statResponse.innerText = `"${debateResult.statistician}"`;
            expertResponse.innerText = `"${debateResult.expert}"`;
            pessimistResponse.innerText = `"${debateResult.pessimist}"`;
            trendResponse.innerText = `"${debateResult.trend}"`;

            // Build Chat Dialogue visually
            chatDialogue.innerHTML = `
                <div style="background: rgba(30, 41, 59, 0.8); padding: 1rem; border-radius: 12px; border-left: 4px solid var(--stat-color);">
                    <strong>📊 Le Statisticien :</strong><br><span style="color:#cbd5e1">${debateResult.statistician}</span>
                </div>
                <div style="background: rgba(30, 41, 59, 0.8); padding: 1rem; border-radius: 12px; border-left: 4px solid var(--expert-color);">
                    <strong>🧠 L'Expert Terrain :</strong><br><span style="color:#cbd5e1">${debateResult.expert}</span>
                </div>
                <div style="background: rgba(30, 41, 59, 0.8); padding: 1rem; border-radius: 12px; border-left: 4px solid #ef4444;">
                    <strong>👿 L'Avocat du Diable :</strong><br><span style="color:#cbd5e1">${debateResult.pessimist}</span>
                </div>
                <div style="background: rgba(30, 41, 59, 0.8); padding: 1rem; border-radius: 12px; border-left: 4px solid #a855f7;">
                    <strong>📈 Le Réseauteur :</strong><br><span style="color:#cbd5e1">${debateResult.trend}</span>
                </div>
            `;

            // Render Final Ticket
            renderTicket(debateResult.ticket);
